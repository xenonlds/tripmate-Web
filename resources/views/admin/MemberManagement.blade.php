...
<head>
  ...
  <title>Member Management - TripMate</title>
...
<body>
          ...
          <tr>
            ...
            <td>
              <div class="operation-buttons">
                <button class="btn btn-details" onclick="viewDetails('{{ $member['tourist_id'] }}')">
                  Details
                </button>
                <button class="btn btn-update" onclick="UpdateMember('{{ $member['tourist_id'] }}')">
                  Update
                </button>
                <button class="btn btn-toggle" onclick="toggleStatus('{{ $member['tourist_id'] }}', '{{ $member['User']['status'] ?? 'inactive' }}')">
                  {{ ($member['User']['status'] ?? 'inactive') === 'active' ? 'Deactivate' : 'Activate' }}
                </button>
              </div>
            </td>
          </tr>
  ...
  <script>
    ...
    function viewDetails(touristId) {
      window.location.href = `/admin/member/${touristId}`;
    }
function UpdateMember(touristId) {
  window.location.href = `/admin/member/${touristId}/edit`;
}
    function addMember() {
      window.location.href = '{{ route("admin.member.create") }}';
    }

    function toggleStatus(touristId, currentStatus) {
      const action = currentStatus === 'active' ? 'deactivate' : 'activate';
      if (confirm(`Are you sure you want to ${action} this member?`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = `/admin/member/${touristId}/toggle-status`;
    ...
    function deleteMember(touristId, name) {
      if (confirm(`Are you sure you want to delete ${name}? This action cannot be undone.`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = `/admin/member/${touristId}`;
